import { resultInterface, userInterface } from "../interfaces.td";
import {
  loginService,
  registerService,
  listService,
...
export const loginCtrl = async (req: Request, res: Response) => {
  try {
    ...
    const { access_token, refreshToken } = result as resultInterface;
...
export const forgotPassword = async (req: Request, res: Response) => {
  try {
    ...
    const newToken = createToken(existingUser as userInterface);
    await emails({
      to: req.body.email,
    ...
    res
      .status(201)
      .send({ message: "new token genrated successfully", token: newToken });
  } catch (error) {
    console.error(error);
  }
};
...
export const resetPassword = async (req: Request, res: Response) => {
  ...
  const payload = JWT.verify(token, process.env.SECRET as Secret) as JwtPayload;
  if (!payload) {
